...
print("✓ Стратификация применена для сохранения баланса классов")
...
for metric in base_metrics.keys():
    ...
    print(
        f"  {metric:10}: база={base_value:.4f}, бутстрап={bootstrap_mean:.4f}, разница={difference:+.4f}"
    )
...
ax1.set_title(
    "Распределение метрик качества\n(100 бутстрап-повторов)",
    fontsize=14,
    fontweight="bold",
)
...
for metric in ["accuracy", "f1", "roc_auc"]:
    ax3.plot(results_df[metric].rolling(window=10).mean(), label=metric, linewidth=2)
ax3.set_title(
    "Скользящее среднее метрик\n(окно=10 повторов)", fontsize=14, fontweight="bold"
)
...
ax5.barh(
    ...
    xerr=std_importances[sorted_idx],
    capsize=3,
)
...
# Анализ стабильности
most_stable_metric = min(
    metrics_history.keys(), key=lambda x: results_df[x].std() / results_df[x].mean()
)
least_stable_metric = max(
    metrics_history.keys(), key=lambda x: results_df[x].std() / results_df[x].mean()
)

print("\nАНАЛИЗ СТАБИЛЬНОСТИ:")
print(
    f"✓ Наиболее стабильная метрика: {most_stable_metric} "
    f"(CV={(results_df[most_stable_metric].std() / results_df[most_stable_metric].mean() * 100):.2f}%)"
)
print(
    f"✓ Наименее стабильная метрика: {least_stable_metric} "
...
print(
    f"✓ Модель демонстрирует {'высокую' if avg_roc_auc > 0.85 else 'умеренную'} воспроизводимость результатов"
)
print("✓ Разброс метрик находится в приемлемых пределах для медицинской диагностики")

print("\nРЕКОМЕНДАЦИИ:")
print("1. Модель можно использовать для предварительной диагностики")
print("2. Для повышения стабильности увеличение размера выборки")
